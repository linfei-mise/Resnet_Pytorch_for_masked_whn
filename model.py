...
class BasicBlock(nn.Module):  # 定义18层和34层的残差结构
    ...
    def __init__(self, in_channel, out_channel, stride=1, downsample=None, **kwargs):  # downsample：虚线残差结构
...
class Bottleneck(nn.Module):  # 定义50层101层和152层的残差结构
    ...
    def __init__(self, in_channel, out_channel, stride=1, downsample=None,
        ...
        self.bn1 = nn.BatchNorm2d(width)  # 开始BN
        # -----------------------------------------
        self.conv2 = nn.Conv2d(in_channels=width, out_channels=width, groups=groups,
                               kernel_size=3, stride=stride, bias=False, padding=1)
        self.bn2 = nn.BatchNorm2d(width)
        # -----------------------------------------
        self.conv3 = nn.Conv2d(in_channels=width, out_channels=out_channel*self.expansion,  # 上述的第3层变化
...
class ResNet(nn.Module):  # 定义ResNet整个网络框架
    ...
    def __init__(self,
                 block,  # 使用的是结构，包含18，34，50，101.152
                 blocks_num,  # 列表结构
                 num_classes=1000,
                 include_top=True,  # 方便在ResNet上搭建更深的网络结构
                 groups=1,
                 width_per_group=64):
