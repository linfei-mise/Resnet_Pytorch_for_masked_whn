...
def read_split_data(root: str, val_rate: float = 0.1):  # root指向Dataset_changes_2文件夹的路径，划分率0.1
    random.seed(0)  # 设置随机种子，保证随机结果可复现，在其他设备上使用时划分的结果一模一样
    assert os.path.exists(root), "dataset root: {} does not exist.".format(root)  # 判断路径是否存在

    # 遍历文件夹，一个文件夹对应一个类别
    hat_class = [cla for cla in os.listdir(root) if os.path.isdir(os.path.join(root, cla))]
    # 排序，保证顺序一致
    hat_class.sort()
    # 生成类别名称以及对应的数字索引
    class_indices = dict((k, v) for v, k in enumerate(hat_class))
    ...
    for cla in hat_class:
        ...
        images = [os.path.join(root, cla, i) for i in os.listdir(cla_path)
                  if os.path.splitext(i)[-1] in supported]  # 后缀名是否支持
        # 获取该类别对应的索引
    ...
    print("{} images were found in the dataset.".format(sum(every_class_num)))  # 打印个数
    ...
    if plot_image:
        # 绘制每种类别个数柱状图
        plt.bar(range(len(hat_class)), every_class_num, align='center')
        # 将横坐标0,1,2,3,4替换为相应的类别名称
        plt.xticks(range(len(hat_class)), hat_class)
        # 在柱状图上添加数值标签
        ...
        # 设置柱状图的标题
        plt.title('hat class distribution')
        plt.show()
